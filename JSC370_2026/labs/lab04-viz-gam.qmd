...
(ggplot(met_july,
  ...
  facet_wrap('~region') +
  labs(x='Elevation Category', y='Dew Point (°C)',
       title='Dew Point by Elevation Category and Region') +
  theme_minimal())

```

Summary:
- The average dew points for low and high elevations in the NE and SE regions have similar distribution. They are also higher compared to the average dew points in the NW and SW regions regardless of elevation category.

- In the NW region, the range of dew points for high elevation is wider than that for low elevation, while the average dew points are approximately the same. 

- In the SW region, the distribution of dew points for low elevation is bimodal. 
...
(ggplot(met_july,
  ...
  labs(x='elev_high_low', y='Proportion of stations',
       title='Proportion of Weather Stations by Elevation Category and Region') +
  theme_minimal())
```
...
(ggplot(met_july,
  ...
  scale_fill_brewer(type='qual', palette='Set2') +
  labs(x='elev_high_low', y='Count of stations',
       title='Counts of Weather Stations by Elevation Category and Region') +
  theme_minimal())

```

Summary:
- In the first barplot with fill positioning, we can see the proportion of stations by region, for high and low elevation categories respectively. For high elevation, NE has the largest proportion, although the four regions do not have drastically different proportions. For low elevation, SE occupies more than 50% of the total number of stations. 

- In the second barplot with dodge positioning, we can see the comparison of the individual counts of stations more clearly. Within the high elevation category, NE has the largest number of weather stations (approximately twice the number of stations in the other 3 regions). Within the low elevation category, the counts are more extreme. The SE region has close to 400,000 stations, whereas the NW region only has less than 50,000. 
...
(ggplot(met_july, aes(x='region', y='dew_point')) +
  ...
  labs(x='Region', y='Dew Point',
       title='Mean Dew Point by Region with Standard Deviation Error Bars') +
  theme_minimal())
```

Summary:
- The SE region has the highest mean dew point, followed by NE, SW, and NW. 

- The SW region has the largest range of dew points within 1 standard deviation within its mean. The SE region has the lowest range.
...
- Report adjusted R2
The adjusted R2 is 0.115. 

- Are the beta coefficients for wind speed and temperature significant?
The beta coefficient for wind speed is -0.6365 and has p < 0.001; the beta coefficient for temperature is 0.3780 and has p < 0.001. They are both significant at the 0.001 level.
...
The pseudo R2 is 0.2868 which is greater than the linear model's adjusted R2 of 0.115.

- What are the EDoF for wind speed and temp?
The EDoF for wind speed is 13.5; for temp it's 16.9.

- Are the smooths for wind speed and temperature significant?
Both the smooths for wind speed and temperature have p-values = 1.11e-16, so both are significant. 
...
The curve for the effect of wind speed on dew point is more wiggly than the curve for the effect of temperature. 
For the wind speed curve, the 95% confidence interval is wider, and the width grows further as wind speed increases. 
For the temperature curve, the 95% confidence interval is closer to the point estimate, and the width of the confidence interval is more constant as x increases.

- Does the smooth term capture meaningful non-linearity?
Yes, numerically the GAM's pseudo R2 os 0.2868 is substantially greater than 0.115 which is the adjusted R2 for the OLS model. Also, since the EDoF for both wind speed and temperature are >> 1 and significant, it indicates that GAM learnt meaningful nonlinear relationships. 
From the partial dependence plots, the fitted curves both show curvature instead of linearity. 
