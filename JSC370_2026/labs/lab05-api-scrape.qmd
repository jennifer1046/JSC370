...
API_KEY = "8e15d2f12a7ed216bf346076fa8fd450"  # Replace with your API key!
...
display(len(merged))
display(len(merged[merged['won_osc']]))
display(len(merged[~merged['won_osc']]))
```
...
display(merged[merged['won_osc']])

# 2. Compare average budget and revenue for Oscar winners vs non-winners
display(merged[merged['won_osc']][['budget','revenue']].mean())
display(merged[~merged['won_osc']][['budget','revenue']].mean())
...
- The median revenue for non-Oscar winning movies is lower than that for Oscar winning movies.
...
- Most movies are able to break even since most points are above the black line. This means their revenue exceeds budget.

- For the points under the break even line, the vast majority of them are not Oscar winning movies. 
...
(
    ggplot(merged[merged['roi'] < 1000000], aes(x='won_osc', y='roi')) +
    geom_boxplot() +
    theme_classic()
)
```
...
- From the boxplots, the median return on investment is higher for Oscar winning movies. The distribution for non Oscar winning movies has a lot of high outliers. 
---
