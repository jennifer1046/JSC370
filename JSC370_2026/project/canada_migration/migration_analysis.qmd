...
g_4d = (
    ...
    + p9.labs(title="Distribution Shift by Period, Faceted by CMA Size", x="", y="Migration rate (per 1,000)")
...
def build_pipeline(feats, model):
    ...
    # latest fit, not theirs (see docs/bugs-and-fixes.md #1).
...
for outcome, feats in outcome_specs:
    ...
    # Store preprocessor + fitted GAM together so Phase 3/4 can reuse them.
...
lag_model_ctors = {
    ...
    # XGBoost n_jobs=-1 is safe (OpenMP threads, shared memory).
...
for outcome, feats in outcome_specs:
    for spec_name, extra_feats in lag_specs.items():
        ...
        for model_name, ctor in lag_model_ctors.items():
            res = score_lag_spec(outcome, feats_with_lag, train_lag, test_lag, ctor)
            res.update({
